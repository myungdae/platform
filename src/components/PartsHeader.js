...
const PartsHeader = () => {
  return (
    <header className="border-b-black relative">
      <a href="/">
        <img
          src="/logo-black.svg"
          alt="데이터센터"
      ...
      <nav className="header-nav max-width">
        {NavItems.slice(0, 9).map((item, index) => (
            <a href={`/facet/${item.replace(/\s+/g, "")}`} key={index}>{item}</a>
        ))}
        <DropdownMenu items={NavItems.slice(9)} />
        <div className="header-nav-side">
          ...
          <div id="auth-container">
            <a href="/login" id="login-btn" className="border-r-black">
              로그인
            </a>
